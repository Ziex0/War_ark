...
bool Player::resetTalents(bool noResetCost)
{
    ...
    uint32 resetCost = 0;
    if (!noResetCost && !CONF_GET_BOOL("NoResetTalentsCost"))
    {
...
void Player::DeleteFromDB(ObjectGuid::LowType lowGuid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    uint32 charDelete_minLvl = CONF_GET_INT("CharDelete.MinLevel");
        ...
        case CHAR_DELETE_REMOVE:
            {
                ...
                if (resultMail)
                {
                    ...
                    do
                    {
                        auto mailFields = resultMail->Fetch();
...
{
    if (!sScriptMgr->CanResurrect(this, restore_percent, applySickness))
        return;
...
bool Player::CanJoinConstantChannelInZone(ChatChannelsEntry const* channel, AreaTableEntry const* zone)
{
    ...
    if (channel->flags & CHANNEL_DBC_FLAG_LFG && CONF_GET_BOOL("LFG.Location.All"))
        return true;
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            else
            {
                ...
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr->GetBaseXP(areaEntry->area_level) * exploration_percent / 100 * CONF_GET_FLOAT("Rate.XP.Explore"));
                }
                else
                {
                    XP = uint32(sObjectMgr->GetBaseXP(areaEntry->area_level) * CONF_GET_FLOAT("Rate.XP.Explore"));
                }
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, int32 honor, bool awardXP)
{
    ...
    */
    ...
    if (CONF_GET_BOOL("PvPToken.Enable"))
    {
        if (!uVictim || uVictim == this || uVictim->HasAuraType(SPELL_AURA_NO_PVP_CREDIT))
            return true;
...
{
    if (value > CONF_GET_UINT("MaxArenaPoints"))
        value = CONF_GET_UINT("MaxArenaPoints");
    SetUInt32Value(PLAYER_FIELD_ARENA_CURRENCY, value);
    if (value)
        AddKnownCurrency(ITEM_ARENA_POINTS_ID);
}

void Player::ModifyHonorPoints(int32 value, CharacterDatabaseTransaction trans /*=nullptr*/)
{
...
void Player::ModifyArenaPoints(int32 value, CharacterDatabaseTransaction trans /*=nullptr*/)
{
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    else if (guid.IsCorpse())                          // remove insignia
    {
        ...
        if (loot->loot_type == LOOT_NONE)
        {
            ...
            bones->loot.gold = uint32(urand(50, 150) * 0.016f * pow(float(pLevel) / 5.76f, 2.5f) * CONF_GET_FLOAT("Rate.Drop.Money"));
        }
...
template WH_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, int32& basevalue, Spell* spell, bool temporaryPet);
template WH_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, uint32& basevalue, Spell* spell, bool temporaryPet);
template WH_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, float& basevalue, Spell* spell, bool temporaryPet);
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= nullptr*/, uint32 spellid /*= 1*/)
{
    ...
    if ((CONF_GET_INT("InstantFlightPaths") == 1 || (CONF_GET_INT("InstantFlightPaths") == 2 && m_isInstantFlightOn)) && !spellid)
    {
...
{
    auto const& itr = m_spellCooldowns.find(spell_id);
...
{
    auto const& itr = m_spellCooldowns.find(spell_id);
    return uint32(itr != m_spellCooldowns.end() && itr->second.end > GameTime::GetGameTimeMS().count() ? itr->second.end - GameTime::GetGameTimeMS().count() : 0);
}
